...
eventBus.registerDlqHandler(async (event, error) => {
  console.error(
    `Event ${event.eventId} failed after retries. Routing to DLQ. Error: ${error.message}`,
  );
});
...
await eventBus.publish(
  'volunteer.created',
  { volunteerId: 'v-100', email: 'john@mevis.io' },
  {
    aggregateId: 'v-100',
